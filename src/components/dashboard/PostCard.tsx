...
const PostCard = ({ post, currentUser, hasReacted, reactionType = "LIKE" }: PostCardProps) => {
  ...
  const [userReactionType, setUserReactionType] = useState<string>(reactionType);
  ...
  const getReactionIcon = (type: string, filled: boolean = false) => {
    ...
    switch (type) {
      case "LIKE":
        return filled ?
            <ThumbsUp className="h-5 w-5" strokeWidth={2} fill="currentColor" /> :
      ...
      case "TROPHY":
        ...
        />;
      default:
        return <ThumbsUp className="h-5 w-5" strokeWidth={filled ? 2 : 1.5} />;
    }
  };
  ...
  const getReactionColor = (type: string) => {
    switch (type) {
      case "LIKE": return "text-blue-600 bg-blue-50";
      case "LOVE": return "text-red-600 bg-red-50";
      case "LAUGH": return "text-amber-600 bg-amber-50";
      case "STAR": return "text-yellow-600 bg-yellow-50";
      case "TROPHY": return "text-purple-600 bg-purple-50";
  ...
  return (
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-b border-gray-100">
          <div className="flex items-center">
            ...
            <div className="ml-3 flex-1 min-w-0">
              <h3 className="font-medium text-gray-900 truncate">{post.author.name}</h3>
            ...
            <div className="flex-shrink-0 ml-2">
              <div className="inline-flex items-center px-2.5 py-1 rounded-full bg-blue-50 text-blue-600">
                <span className="text-xs font-medium truncate max-w-[100px]">{post.group.name}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5">
          {/* Post Text */}
          <p className="text-gray-700 text-sm sm:text-base mb-4">{post.content}</p>
          ...
          <div className="pt-2 border-t border-gray-100 flex justify-between items-center">
            ...
            <div className="relative">
              <button
                  ...
                  aria-label={userHasReacted ? `Reacted with ${userReactionType.toLowerCase()}` : "React to post"}
              >
                {userHasReacted
              ...
              {showReactionSelector && (
                  ...
                  >
                    {(["LIKE", "LOVE", "LAUGH", "STAR", "TROPHY"] as string[]).map((type) => (
                        ...
                        >
                          {getReactionIcon(type, userHasReacted && userReactionType === type)}
                        </button>
                    ))}
                  </div>
              )}
